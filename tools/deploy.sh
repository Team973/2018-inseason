...
deploy () {
    if [ $? -eq 0 ]; then
        ...
        ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null "$TARGET_USER@$TARGET" "rm -f $TARGET_DIR/FRCUserProgram" > /dev/null 2>&1
        echo "Copying over new program..."
        scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null "$PROGRAM" "$TARGET_USER@$TARGET:$TARGET_DIR/FRCUserProgram" > /dev/null 2>&1
...
ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null "$TARGET_USER@$TARGET" true > /dev/null 2>&1
deploy
